...
Mutation-validating a test (temporarily breaking the code under test to confirm the test fails, then reverting) proves the test can fail — not that it only fails for the right reason. Plant and revert a mutation inside a single tool call; when that is not possible, run it on a `git archive [SHA]` copy outside the worktree, never in the shared tree, where another agent reads it as a regression. The mutant must be killed under every selection/invocation mode the changed code exposes, not only the default — a mode-filtered path spared by the default mode's run is the incident class. Every test you mutation-validate also gets repeat runs (default N=10) at elevated parallelism (e.g. `--test-threads` at roughly double the runner's default). Report both numbers in your artifact's `summary`, in this fixed format: `mutation: killed X/X; stability: Y/N at T threads`. That field and `qa_metadata` are the only carriers read as your own measurement; the same citation living only inside a blocker or suggestion is treated as evidence you are quoting and is not checked, so putting your own pairing numbers there hides them from the gate. A test that passes mutation but fails any stability run is a concurrency-sensitive finding, never a pass.
