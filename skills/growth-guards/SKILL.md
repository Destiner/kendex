...
--no-verify` is the deliberate bypass. `kendex add` and `kendex refresh` run
the installer, `kendex remove growth-guards` runs `--uninstall` first, and
`kendex check` folds in `--check`'s read-only verdict (0 armed — in
