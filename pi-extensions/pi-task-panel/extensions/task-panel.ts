...
const TASK_CONTEXT_TYPE = "kendex-task-panel:context";
const TASK_COMPLETE_MESSAGE_TYPE = "kendex-task-panel:complete";
const WIDGET_KEY = "kendex-task-panel";
...
export default function taskPanel(pi: ExtensionAPI): void {
	...
	if (settingBoolean("takeoverCtrlT", false)) {
		pi.registerShortcut("ctrl+t", { description: "Toggle task panel (kendex takeover)", handler: async (ctx) => toggle(ctx as ExtensionContext) });
	}
