...
const INSTALL_SYMBOL = Symbol.for("kendex.pi-questions.installed");
const CONFIG_ID = "@vanillagreen/pi-questions";
const SERVICE_SYMBOL = Symbol.for("kendex.pi-questions.service");
const QOL_NOTIFICATION_SERVICE_SYMBOL = Symbol.for("kendex.pi-qol.notification-service");
const KENDEX_MODAL_LOCK_SYMBOL = Symbol.for("kendex.pi.modal-lock");
const QUESTION_OPENED_EVENT = "kendex:pi-questions:opened";
...
function kendexModalLock(): kendexModalLock {
...
function iskendexModalActive(): boolean {
	return kendexModalLock().depth > 0;
}

function acquirekendexModalLock(): () => void {
	const lock = kendexModalLock();
	lock.depth += 1;
	let released = false;
	return () => {
		if (released) return;
