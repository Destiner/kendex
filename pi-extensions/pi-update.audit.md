...
- **`pi.sendMessage(..., { triggerTurn: false })` no longer steers an active run ([#8022](https://github.com/earendil-works/pi/pull/8022))** — all nine call sites of ours are record-only by intent: `pi-hooks/extensions/hooks.ts:50` (drift report at session start), `pi-task-panel/extensions/task-panel.ts:1283`, `pi-qol/extensions/qol.ts:872` and `qol/session-search/{context.ts:135,index.ts:91,115}`, `pi-codex-minimal-tools/src/background-image-generation.ts:481,520` and `provider-shim.ts:1941,1957`. Pi now matches the intent; before the fix these could steer an active run we never meant to steer, so the change is strictly protective.
