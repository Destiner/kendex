 ...
 *   KENDEX_AGENT_END_WATCHDOG=0          disables the watchdog entirely.
 *   KENDEX_AGENT_END_WATCHDOG_GRACE_SEC  grace before firing (default 10s).
 */
...
export function buildSyntheticOutbox(agentName: string, taskId: string): SyntheticOutboxPayload {
	return {
		...
		synthetic: true,
		notes: "Synthesized by settled-run watchdog (kendex#66). Treat as needs_completion; the child agent did not write a real outbox.",
	};
}
...
export function watchdogEnabledFromEnv(env: NodeJS.ProcessEnv = process.env): boolean {
	const raw = env.KENDEX_AGENT_END_WATCHDOG?.trim();
...
export function watchdogGraceMsFromEnv(env: NodeJS.ProcessEnv = process.env): number {
	const raw = env.KENDEX_AGENT_END_WATCHDOG_GRACE_SEC?.trim();
