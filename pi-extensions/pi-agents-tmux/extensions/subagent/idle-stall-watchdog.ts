...
//   KENDEX_STALL_WATCHDOG=0                disables the watchdog entirely.
...
export function buildStallSyntheticOutbox(
	...
	return {
		...
		synthetic: true,
		notes:
			"Synthesized by idle-stall watchdog (kendex#63 workaround). Treat as needs_completion; the child agent may have stalled after auto-compaction.",
	};
}
...
export function stallWatchdogEnabledFromEnv(env: NodeJS.ProcessEnv = process.env): boolean {
	const raw = env.KENDEX_STALL_WATCHDOG?.trim();
...
export function stallWatchdogIntervalMsFromEnv(env: NodeJS.ProcessEnv = process.env): number {
	const raw = env.KENDEX_STALL_WATCHDOG_INTERVAL_SEC?.trim();
...
export function stallWatchdogThresholdMsFromEnv(env: NodeJS.ProcessEnv = process.env): number {
	const raw = env.KENDEX_STALL_WATCHDOG_THRESHOLD_SEC?.trim();
	const parsed = raw ? Number(raw) : Number.NaN;
