...
function agentCurrentFrontmatterEdit(agent: AgentConfig): AgentFrontmatterEdit {
	...
	if (!iskendexManagedAgentFile(agent)) return current;
	const tomlPath = kendexTomlPathForAgent(agent, process.cwd());
...
function editableAgentFrontmatterText(agent: AgentConfig): string {
	...
	const lines = [
		...
		"# For kendex-managed agents, this writes [agent-frontmatter.pi] in kendex.toml.",
...
function iskendexManagedAgentFile(agent: AgentConfig): boolean {
	try {
		const raw = fs.readFileSync(agent.filePath, "utf-8");
		return raw.includes("Never edit this file directly") && raw.includes("kendex refresh");
	} catch {
		return false;
	}
}
...
function upsertAgentFrontmatterToml(content: string, agentName: string, edit: AgentFrontmatterEdit): string {
	...
	if (!span) {
		...
		const block = ["", "# Pi-specific frontmatter values. The Pi /agents popup edits", "# kendex-managed entries in this file, then `kendex refresh` applies them.", section, ""];
...
export async function editAgentFrontmatterOverrides(ctx: ExtensionContext, agent: AgentConfig): Promise<string | undefined> {
	...
	if (iskendexManagedAgentFile(agent)) {
		const tomlPath = kendexTomlPathForAgent(agent, ctx.cwd);
		if (!tomlPath) throw new Error(`Could not locate kendex.toml for kendex-managed agent ${agent.name}.`);
