...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			if (allowedList.length === 0) {
				return {
					content: [{ type: "text", text: `delegate_subagent: ${callerName} has no allowed-subagents configured. Add a list to the agent frontmatter (kendex.toml [agent-frontmatter.pi]) or use the full subagent tool from a parent session.` }],
					details: makeDetails("single")([]),
					isError: true,
				};
			}
