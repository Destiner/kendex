...
test("spinner setting defaults and project override precedence", () => {
	withTempPiUserDir(() => {
		...
		writeFileSync(join(userDir, "settings.json"), JSON.stringify({
			kendex: { extensionManager: { config: { "@vanillagreen/pi-agents-tmux": { animateSpinners: false } } } },
		}));
...
test("Monitor detail navigation can reach the Transcript subtab (kendex#611)", async () => {
