...
describe("resource-control spawn planning", () => {
	...
	test("systemd-run service plan keeps command and cwd as argv, not shell-quoted text", () => {
		...
		expect(plan.metadata).toEqual({ mode: "systemd-run", requestedMode: "auto", unitName: "kendex-pi-bg-bg-7-123456.service", warning: undefined });
		...
		expect(plan.args).toContain("--unit=kendex-pi-bg-bg-7-123456.service");
...
describe("resource-control stop and restore semantics", () => {
	test("systemd unit stop persists enough metadata to stop actual workload", () => {
		...
		const metadata = { mode: "systemd-run" as const, requestedMode: "auto" as const, unitName: "kendex-pi-bg-bg-7.service" };
		...
		expect(calls).toEqual([
			{ command: "systemctl", args: ["--user", "stop", "--no-block", "kendex-pi-bg-bg-7.service"] },
			{ command: "systemctl", args: ["--user", "kill", "--signal=SIGKILL", "kendex-pi-bg-bg-7.service"] },
		]);
	});
	...
	test("restore keeps a systemd unit running when the unit is active even if wrapper pid probe is gone", () => {
		const restored = restoredTaskFromSnapshot(fakeSnapshot({
			resourceControl: { mode: "systemd-run", requestedMode: "auto", unitName: "kendex-pi-bg-bg-7.service" },
		}), {
			sessionId: "session-A",
			identityProbe: () => null,
