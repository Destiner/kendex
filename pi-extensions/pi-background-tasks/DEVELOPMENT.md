...
- `notifyMode` — `always`, `transition`, `first-match-only`. When omitted, `resolveNotifyMode()` picks `first-match-only` if `notifyPattern` is set, `transition` otherwise (kendex#210). Pass `"always"` explicitly to opt into every-output wakes.
...
- The legacy `newOutputTail` field was removed in kendex#210; for output wakes the wake-time `outputTail` IS the new unseen excerpt, and for exit wakes it is the trailing portion of full output.
...
`createPersistence().persistSnapshots()` is the only writer of the `kendex-background-tasks:state` JSONL entry. It enforces two guards before calling `pi.appendEntry`:
...
## Bounded tool-result details (kendex#187)
