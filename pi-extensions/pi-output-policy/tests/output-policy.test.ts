...
describe("tool_result handler (default-on sanitization & metadata)", () => {
		...
		await new Promise<void>((resolveTest) => {
			withConfig({}, (cwd) => {
				...
				fake.fire("tool_result", {
					...
					expect(result.details.kendexOutputPolicySanitized).toBeTruthy();
	...
	test("kendexOutputPolicy meta is attached when text is truncated", async () => {
		await new Promise<void>((resolveTest) => {
			withConfig({}, (cwd) => {
				const fake = createFakePi();
				outputPolicy(fake.pi);
				...
				fake.fire("tool_result", {
					...
					expect(result.details.kendexOutputPolicy[0].artifactPath).toBeTruthy();
		...
		await new Promise<void>((resolveTest) => {
			withConfig({}, (cwd) => {
				runWithHandlers(cwd, async (fake, ctx, oversized) => {
					...
					const meta1 = r1.details.kendexOutputPolicy[0];
