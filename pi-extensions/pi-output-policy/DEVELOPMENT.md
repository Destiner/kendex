...
When `details` are sanitized, the result carries a `kendexOutputPolicySanitized` marker (and capped arrays/objects include a sentinel string) so consumers can detect the truncation.
