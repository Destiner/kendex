...
Any per-knob value you set in `kendex.extensionManager.config["@vanillagreen/pi-output-policy"]` overrides the mode default; unset knobs follow the mode.
...
- Sanitized `details` carry a `kendexOutputPolicySanitized` marker (capped arrays/objects include a sentinel string) so integrators inspecting tool-result payloads can detect it — shape details in `DEVELOPMENT.md`.
...
kendex add vanillagreencom/kendex --pi-extension pi-output-policy --harness pi -y
```
