...
A separate dispatch guard handles names that cannot belong to Pi's bridged manifest without broadening `isChildExecutedTool`, which is also used while declaring Pi tools. A non-bridge `mcp__<server>__<tool>` name belongs to a filesystem-loaded child server, while a bare `bash`/`read`/`edit`/`write` is an unroutable SDK naming slip whenever the bridge supplied a manifest. Neither becomes a Pi tool call; bridge-owned `mcp__custom-tools__*` names and legacy empty-manifest behavior remain unchanged.
