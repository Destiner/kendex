...
Pi can call one provider instance concurrently for parent sessions, subagents, and sibling conversations. Each `streamSimple` request therefore enters an `AsyncLocalStorage` lane (`src/request-lane.ts`) keyed by `SimpleStreamOptions.sessionId`. All request-owned bridge state resolves through that lane: the active `QueryContext`, Pi stream, pushed-context stack, watchdog owner, and Claude session record. Promise continuations, timers, Agent SDK iterators, and MCP tool handlers born inside the request inherit the lane, so nested asynchronous work cannot observe a sibling request's state. Listeners on objects the host owns do not: an `AbortSignal` listener runs in the aborter's async context, so the abort handler re-enters its own lane explicitly via `runInRequestLane` — follow that pattern for any callback registered on a host-owned emitter. Presence is checked against `undefined`, not truthiness: even an empty string is a named lane rather than the direct-host fallback, and `runInRequestLane(undefined, …)` always selects the default lane, even from inside a named one.

Parent and child agents may load separate copies of the extension module while the process continues using the primary instance's registered provider closure. The `AsyncLocalStorage` carrier plus the shared-session, query-lane, started-lane, and pending-persist registries therefore live on `globalThis` under versioned symbols; child lifecycle handlers and the primary provider observe the same lane identity and per-session records. `session_shutdown` removes only that session's entries and cancels only that `SessionManager`'s pending session-marker persist, allowing short-lived child lanes to be collected without clearing a still-running parent or sibling; the entries removed are those recorded for that `SessionManager` at its `session_start` (a `WeakMap` keyed by manager, so overlapping parent/child starts keep separate entries and a start and shutdown handled by different module instances — a `/reload` mid-session — still agree), because an in-memory session (`pi --no-session`) forks by mutating the same `SessionManager`'s id before shutdown fires. Requests that carry `cacheRetention: "none"` — Pi's compaction and branch-summary one-shots, which also arrive with a fresh `sessionId` per call and never see a `session_shutdown` — release their lane as soon as the request settles, so nothing is retained for them. The default lane exists only for direct hosts that omit `sessionId` and can never be claimed by the first named lane.

Code that reads or mutates request-owned state must use the accessors in `bridge-state.ts`, `query-state.ts`, and `session-persistence.ts` rather than introducing another process-global mutable binding. Regression coverage lives in `tests/unit-session-lanes.mjs` and `tests/unit-foreign-conversation.mjs`, including overlapping parent/child requests, sibling abort isolation, aborts issued from another lane's context, cross-module access, per-session cleanup (including the in-memory fork case, a shutdown handled by a different module instance, per-manager persist cancellation, and `cacheRetention: "none"` one-shots), and independent parallel tool-result loops.
...
- Queued results whose handler has not fired by the next child message boundary are parked there with a `stale_queued_tool_results_parked` diagnostic, instead of poisoning every later mismatch report for the query. Parked results stay consumable: the SDK staggers handler invocations, so a handler can legitimately fire after the boundary and still collect its real result (kendex#1469). A result nobody ever collects belongs to a call the SDK abandoned client-side.
...
A separate dispatch guard handles names that cannot belong to Pi's bridged manifest without broadening `isChildExecutedTool`, which is also used while declaring Pi tools. A non-bridge `mcp__<server>__<tool>` name belongs to a filesystem-loaded child server, while any bare tool name is an unroutable naming slip whenever the bridge supplied a manifest. Neither becomes a Pi tool call; bridge-owned aliases must resolve to that manifest, the MCP-resource audit spellings remain mirrored, and legacy empty-manifest behavior remains unchanged.

The same never-mirror rule covers a second, smaller class: Claude Code's own in-process meta-tools (`ToolSearch` and `ScheduleWakeup` — `isChildInternalTool`, matched by **exact name**, never by prefix). The two MCP-resource built-ins (`ListMcpResources`/`ReadMcpResource`, which the SDK renames to `ListMcpResourcesTool`/`ReadMcpResourceTool` on the stream side) are deliberately NOT in this class: a resource read is a real account-surface access and both consumer hosts audit it through the Pi mirror, so it stays mirrored (kendex#1007). They surface when connectors are enabled, because connector tools are deferred behind `ToolSearch`. Mirroring one made Pi dispatch a tool it does not have and deliver an error result for an id no MCP handler ever claimed; the result queued in `pendingResults` until the reaper dropped it — a "dropped 1 tool result(s) whose handler never matched (ToolSearch)" warning and a phantom failed tool call per discovery, plus a spurious pi-turn boundary (kendex#980). Unlike connectors, these calls are tool plumbing rather than account-data access, so they are also excluded from the connector-call audit below; connector-only concerns key on the narrow `isConnectorTool`.
...
- Rate-limit errors are deduplicated before user notification. The bridge emits `kendex:rate-limit` so `pi-qol` can opt into reset-time auto-resume.
