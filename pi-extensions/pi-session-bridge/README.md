...
kendex add vanillagreencom/kendex --pi-extension pi-session-bridge --harness pi -y
```
...
Local Pi extensions can publish activity updates that `pi-bridge stream` exposes to connected clients. This is used by kendex extensions for agent progress, background tasks, and questions without adding messages to the conversation. Contributor-facing broker details are in [`DEVELOPMENT.md`](./DEVELOPMENT.md).
