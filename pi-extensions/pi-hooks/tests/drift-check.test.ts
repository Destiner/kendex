...
const REPORT = "kendex drift — project scope:\n  1 outdated — run `kendex refresh` to update:\n    ! orch (skill)";
...
describe("drift delivery", () => {
	test("an unexpected throw is reported, never mistaken for a clean install", async () => {
		...
		expect(sent).toEqual(["kendex check could not run: spawn exploded; drift status unknown"]);
	});

	test("a non-Error throw still yields a diagnostic", async () => {
		const sent: string[] = [];
		await deliverDrift(Promise.reject("nope"), (m) => sent.push(m));
		expect(sent).toEqual(["kendex check could not run: nope; drift status unknown"]);
	});
		...
		await withFake("1", REPORT, async ({ root }) => {
			...
			try {
				await withIsolatedSettings(root, async () => {
					...
					expect(sent).toEqual([
						{
							message: { customType: "kendex-drift", content: REPORT, display: true },
