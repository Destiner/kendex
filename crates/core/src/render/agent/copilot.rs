...
pub fn generate(agent: &EffectiveAgent) -> RenderedAgent {
    ...
    match &agent.permissions {
        PermissionIntent::AllowOnly { allow, .. } => {
            let allow: Vec<String> = allow.iter().map(|tool| copilot_tool_name(tool)).collect();
        ...
        // Copilot's agent frontmatter carries an allowlist and nothing else,
        // and completing one from a deny list would take the agent's own
        // tools away the moment Copilot grows a built-in it never named.
        PermissionIntent::DenyExtra(deny) => warnings.push(RenderWarning::with_fix(
            format!(
                "Copilot agents take a tool allowlist and no deny list, so this agent keeps access to {}",
                deny.join(", ")
            ),
